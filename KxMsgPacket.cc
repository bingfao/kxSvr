...
void KxMsgPacket_Basic::getvecBuffer(std::vector<asio::const_buffer> &vec_buf)
{
	if (m_msgHeader.nTypeFlag == cst_Resp_MsgType)
		...
		vec_buf.push_back(asio::buffer(&m_msgHeader, sizeof(KxMsgReqHeader)));
	if (getMsgBodyBuf() && getBodyLen())
		vec_buf.push_back(asio::buffer(getMsgBodyBuf(), getBodyLen()));
}

void KxMsgPacket_Basic::calculate_crc()
{
